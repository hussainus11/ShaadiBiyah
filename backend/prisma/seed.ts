...
async function main() {
  ...
  // Seed sample matrimonial profiles (only if they don't exist)
  const existingMatrimonialProfiles = await prisma.matrimonialProfile.findMany({
    where: {
      OR: [
  ...
  if (existingMatrimonialProfiles.length === 0) {
    ...
    console.log('✅ Created 3 matrimonial profiles');
  } else {
    console.log('ℹ️  Matrimonial profiles already exist, skipping...');
  }
  ...
  const existingBookings = await prisma.booking.findMany({
    where: {
      OR: [
        { userId: user1.id, vendorId: vendorProfile1.id },
        { userId: user2.id, vendorId: vendorProfile2.id }
      ]
    }
  });
  ...
  if (existingBookings.length === 0) {
    ...
    console.log('ℹ️  Bookings already exist, skipping...');
  }
